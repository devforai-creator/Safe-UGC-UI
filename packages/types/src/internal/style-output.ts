/**
 * Shared renderer-equivalent style output helpers.
 *
 * This module resolves style DSL objects into plain CSS-like records without a
 * React dependency so the validator can measure the same render output that the
...
export function countResolvedCssBytes(style: ResolvedCssStyle | undefined): number {
...
// ---------------------------------------------------------------------------
// Main: mapStyleToRenderOutput
// ---------------------------------------------------------------------------

/**
 * Convert a UGC style object into the plain CSS-like record that the renderer
 * ultimately consumes.
 */
export function mapStyleToRenderOutput(
  ...
  for (const side of ['borderTop', 'borderRight', 'borderBottom', 'borderLeft'] as const) {
    ...
    if (resolvedSide) {
      css[side] = borderToCss(resolvedSide);
    }
  }
...
export function countResolvedStyleOutputBytes(
  style: Record<string, unknown> | undefined,
  state: Record<string, unknown>,
  locals?: Record<string, unknown>,
): number {
  return countResolvedCssBytes(mapStyleToRenderOutput(style, state, locals));
}
