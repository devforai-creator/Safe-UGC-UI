...
- Published packages are `@safe-ugc-ui/types`, `@safe-ugc-ui/schema`, `@safe-ugc-ui/validator`, and `@safe-ugc-ui/react` at `3.0.1`.
...
- 로컬 개발, CI, `publish.yml`은 Node `24`와 pnpm `11.17.0` 기준을 공유하며, publish workflow는 npm trusted publishing에도 Node `24`를 사용한다.
