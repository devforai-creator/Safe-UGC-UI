...
- Published packages are `@safe-ugc-ui/types`, `@safe-ugc-ui/schema`, `@safe-ugc-ui/validator`, and `@safe-ugc-ui/react` at `1.3.1`.
...
- 1.2.0은 `npm publish`로 잘못 나갔으므로 사용하지 않는다. 현재 유효한 최신 버전은 `1.3.1`이다.
